...
def decoratore_func(fun):
    def wrapper(*args):
        ...
        if args in p.keys():  # если наш аргумент равен ключу из переменной "р" то мы выводим значение этого ключа
            print(p.setdefault(args))
        else:
            return fun(*args)
...
def func(*args: int):
    ...
    x = sum(args)  # Суммируем каждое число друг с другом аказанных в аргументе функции
    p[args] = x  # Добовляем в нашу переменную ключ-значение. Где ключь - это аргументы функции, а значение сумма этих
    # аргументов
    print(x)
